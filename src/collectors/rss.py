...
def _candidate_from_entry(entry: Any, source: str, feed_url: str) -> Candidate:
    link = entry.get("link", "")
    ...
    return Candidate(
        ...
        raw_text=" ".join([title, description, " ".join(tags)]),
...
def collect_producthunt(config: dict[str, Any]) -> list[Candidate]:
    ...
    for feed_url in source_config.get("feeds", []):
        ...
        candidates.extend(
            _candidate_from_entry(entry, "Product Hunt", feed_url)
...
def collect_news(config: dict[str, Any]) -> list[Candidate]:
    ...
    for feed_url in source_config.get("feeds", []):
        ...
        candidates.extend(
            _candidate_from_entry(entry, feed_title, feed_url)
